...
from ross2.slam.vision import stream_local_frames
...
async def live_feed():
    return StreamingResponse(
        stream_local_frames(camera_index=0, enable_detection=False),
        media_type="multipart/x-mixed-replace; boundary=frame",
    )


# ---------------------------------------------------------
# ENDPOINT 2: The heavy, annotated detection stream
# ---------------------------------------------------------
@server_app.get("/human_detection")
async def human_detection_feed():
    return StreamingResponse(
        stream_local_frames(camera_index=0, enable_detection=True),
        media_type="multipart/x-mixed-replace; boundary=frame",
    )
...
def stream_server(port: int = 8080):
    ...
    if active_ip != "127.0.0.1":
        ...
        typer.echo(f"  Detection Stream:       http://{active_ip}:{port}/human_detection")
    else:
        typer.echo(f"  Raw Video Stream:       http://localhost:{port}/live")
        typer.echo(f"  Detection Stream:       http://localhost:{port}/human_detection")
    typer.echo("")
